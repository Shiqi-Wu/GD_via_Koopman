...
model_ref.add(Dense(units=128, activation='relu'))
model_ref.add(Dense(units=128, activation='relu'))
...
model_ref.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=1e-2),
              loss=loss_fn)

model_ref.fit(x_train, y_train, batch_size = 128, epochs = 1, verbose=2)
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % 500 == 0:
        ...
        np.save('learn_output/weight_32_%d' %((epoch + 1)/128),weight_record)
        
        del weight_record
        gc.collect()
        weight_record = []

# model_ref.save_weights('./checkpoints/32_checkpoint')